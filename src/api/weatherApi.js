...
export async function geocodeCity(city) {
  ...
  if (!response.ok) {
    if (response.status >= 400 && response.status < 500) {
      throw new Error(`Город "${city}" не найден (ошибка клиента ${response.status})`);
    }
...
export async function getWeatherForecast(lat, lon, days) {
  ...
  if (!response.ok) {
    if (response.status >= 400 && response.status < 500) {
      throw new Error(`Ошибка клиента при получении прогноза (${response.status})`);
    }
    if (response.status >= 500) {
  ...
  if (!data.daily || !data.daily.time || !data.daily.time.length) {
    throw new Error('Некорректные данные прогноза: отсутствуют ежедневные данные');
  }
